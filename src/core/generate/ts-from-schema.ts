...
const getByField = (
    ...
    if (isAny) {
        ...
        return getTypescriptFromEnum(dataField, compilation);
    } else {
        throw new Error(`Unknown schema type: "${dataField.type}"`);
    }
};

export function getTypescriptFromEnum(dataField: ISchemaData, compilation: Compilation): string {
...
const getIncludedSchema = (
    ...
    return getLargestIncluded(included);
};
...
const getLargestIncluded = (included: BuiltInterface[]) => {
