...
import { Models, sModelA, sModelB, useMockApp } from '../mock/app';
import { App, Schema, sIdInput } from '../../src/core';
...
function initApp(app: App<Models>) {
    app.addEndpoint({
        ...
        run: async (system, input) => {
            return await system.models.modelA.create(input);
        }
    });
    ...
    app.addEndpoint({
        ...
        path: '/modelB',
        inputSchema: sModelB,
        outputSchema: Schema.withInstance(sModelB),
        isLoggedIn: true,
        run: async (system, input) => {
            return system.models.modelB.create(input);
        }
    });
    ...
    app.addEndpoint({
        ...
        isLoggedIn: true,
        run: async (system, input) => {
            return system.models.modelB.getById(input.id);
        }
    });
    ...
    app.addEndpoint({
        ...
        isLoggedIn: true,
        run: async (system) => {
            return system.models.modelB.getAll();
        }
    });
    ...
    app.addEndpoint({
        method: 'PUT',
        path: '/modelB/:id',
        inputSchema: Schema.withIdPartial(sModelB),
        outputSchema: Schema.withInstance(sModelB),
        isLoggedIn: true,
        run: async (system, { id, ...input }) => {
            return system.models.modelB.updateById(id, input);
        }
    });
    ...
    app.addEndpoint({
        ...
        isLoggedIn: true,
        run: async (system, input) => {
            return system.models.modelB.deleteById(input.id);
        }
    });
}
